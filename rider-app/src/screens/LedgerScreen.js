...
const mock = [
  ...
  { id: 'Trip 2', time: '12:30-14:00', raw: 18.0, ekm: 14.2, rs: 21.30, verified: '1/2' },
];
    ...
    <View style={{ flex:1, backgroundColor:'white', padding:16 }}>
      ...
      <FlatList data={mock} keyExtractor={i=>i.id} renderItem={({item}) => (
        <View style={{ borderWidth:1, borderColor:'#E2E8F0', borderRadius:12, padding:12, marginTop:10 }}>
          ...
          <View style={{ flexDirection:'row', gap:12, marginTop:8 }}><Text style={{ color:'#334155' }}>Raw {item.raw} km</Text><Text style={{ fontWeight:'700', color:'#0F172A' }}>eKM {item.ekm}</Text><Text style={{ fontWeight:'700', color:'#2563EB' }}>Rs.{item.rs}</Text></View>
      ...
      <View style={{ backgroundColor:'#EFF6FF', padding:12, borderRadius:12, marginTop:12, borderWidth:1, borderColor:'#DBEAFE' }}>
        ...
        <Text style={{ color:'#64748B', fontSize:11, textAlign:'center', marginTop:4 }}>Map Replay pe tap karo — green verified / grey void + selfie stamp dikhega.</Text>
      </View>
    </View>
  );
}
